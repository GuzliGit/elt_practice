...
void start_processing(const int args_count, char** args)
{
    ...
    sem_ids = malloc(sizeof(int) * (args_count / 2));
    for (int i = 0; i < args_count; i+=2)
    {
        key_t key = ftok(args[i], i);
        if ((sem_ids[i / 2] = semget(key, 1, IPC_CREAT | 0666)) == -1)
        {
            perror("semget error");
            exit(EXIT_FAILURE);
        }
        semctl(sem_ids[i / 2], 0, SETVAL, 1);
    }
    ...
    for (int i = 0; i < args_count; i+=2)
    {
        ...
        prod_files[i / 2].semid = sem_ids[i];
