...
void execute(Instruction instrs[], int count)
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        switch (pid = fork()) 
        {
            ...
            case 0:
                if (instrs[i].input_file) 
                {
                    int fd = open(instrs[i].input_file, O_RDONLY);
                    if (fd == -1)
                    {
                ...
                if (instrs[i].output_file) 
                {
                    int flags = O_WRONLY | O_CREAT | (instrs[i].is_append ? O_APPEND : O_TRUNC);
                    mode_t old_umask = umask(0);
                    int fd = open(instrs[i].output_file, flags, 0666);
                    umask(old_umask);
                    if (fd == -1) 
                    {
