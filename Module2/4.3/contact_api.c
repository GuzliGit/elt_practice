...
int export_contacts(const char* filename)
{
    ...
    int count = get_contacts_count();
    fprintf(file, "Телефонная книга (%d контактов)\n", count);

    for (int i = 0; i < count; i++) 
    {
        const Contact* contact = get_contact_by_id(i);
        
        fprintf(file, "%d) %s %s %s\n", i + 1, contact->l_name, contact->f_name, contact->patronymic);
        
        if (strlen(contact->w_info.name) > 0) 
        {
            fprintf(file, "Место работы: %s %s %s\n", contact->w_info.name, contact->w_info.address, contact->w_info.post);
        }

        if (contact->emails_count > 0) 
        {
            fprintf(file, "Адреса почты:\n");
            for (int j = 0; j < contact->emails_count; j++) 
            {
                fprintf(file, " %s|", contact->email[j]);
            }
            fprintf(file, "\n");
        }
        ...
        if (contact->phones_count > 0) 
        {
            fprintf(file, "Телефоны:\n");
            for (int j = 0; j < contact->phones_count; j++) 
            {
                fprintf(file, " %s|", contact->phone_number[j]);
            }
            fprintf(file, "\n");
        }
